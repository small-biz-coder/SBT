...
async function fetchDecks() {
  const res = await fetch('https://api.southbranchtrades.com//wp-json/wp/v2/pages/9');
  if (!res.ok) throw new Error('Failed to fetch decks');
  return parseWPProjects(await res.json());
}
//garages
async function fetchGarages() {
  const res = await fetch('https://api.southbranchtrades.com//wp-json/wp/v2/pages/27');
  if (!res.ok) throw new Error('Failed to fetch garages');
  return parseWPProjects(await res.json());
}
//additions
async function fetchAdditions() {
  const res = await fetch('https://api.southbranchtrades.com//wp-json/wp/v2/pages/35');
  if (!res.ok) throw new Error('Failed to fetch additions');
  return parseWPProjects(await res.json());
}
//misc
async function fetchMisc() {
  const res = await fetch('https://api.southbranchtrades.com//wp-json/wp/v2/pages/43');
